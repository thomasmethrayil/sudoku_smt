...
let parse_sudoku_string sudoku_string puzzle_number =
  ...
  in
  if not (List.length sanitized_string = row_length * row_length)
  then (
    let err_str =
      Printf.sprintf
    ...
    | Some _, Some _ ->
      let err_str =
        Printf.sprintf
          "Sudoku input contains both 0 and '-' to denote missing numbers in puzzle no : \
           %u. Please fix."
          puzzle_number
      in
      failwith err_str
    | _ ->
      sanitized_string
      |> List.map ~f:(fun c -> if Char.( = ) '0' c then None else Char.get_digit c)
      |>
...
let get_subgrid_coordinates grid_dim =
  ...
  let subgrid_squares =
    ...
    |> List.map ~f:(fun (x, y) ->
      ...
      |> List.fold ~init:[] ~f:(fun acc (x1, y1) ->
        if x1 - x = subgrid_length && y1 - y = subgrid_length
        then ((x, y), (x1, y1)) :: acc
        else acc))
  ...
  |> List.map ~f:(fun coords ->
    ...
    |> List.fold ~init:[] ~f:(fun acc x ->
      columns coords |> List.fold ~init:acc ~f:(fun acc y -> (x, y) :: acc)))
;;

let get_subgrid_constraints expr_grid grid_length ctx =
  get_subgrid_coordinates grid_length
  |> List.map ~f:(fun subgrid -> subgrid |> List.map ~f:(fun (x, y) -> expr_grid.(x).(y)))
...
let solve_sudoku unsolved_grid =
  ...
  for index = 0 to grid_dimension - 1 do
    ...
    for colIndex = 0 to grid_dimension - 1 do
      add
        solver
        [ make_cell_constraints unsolved_grid expr_grid index colIndex z3_context ]
    done;
  ...
  |> Array.map ~f:(fun expr_array ->
    ...
    |> Array.map ~f:(fun ex ->
      ...
      | Some interpretation ->
        ...
        if Char.is_digit interpreted_string.(0)
        then Char.get_digit interpreted_string.(0)
        else None
      | None -> None))
;;
...
let load_puzzle_and_solve sudoku_string puzzle_number_opt =
  ...
  | Z.SATISFIABLE, None ->
    Stdio.print_string
      "Solver did not return an assignment despite the existence of satisfying assignments\n"
